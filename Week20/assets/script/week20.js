...
function makeOne() {
	const resultOne = document.getElementById('result1');z
	try {
...
function makeFour() {
	...
	try {
		const a = 2;
		const b =3;
		resultFour.textContent = `${a} * ${b} = ${a * b}`;
		console.log(a * b);
	} catch (error) {
...
function makeFive() {
	let inputData = -1;
	if (inputData < 0)
		throw new Error (`${inputData} меньше нуля`)
...
function makeTen() {
	...
	try {
		...
		} else 	document.getElementById('.result7').textContent = 'Email-адрес корректен';
	} catch (error) {
	console.error('Ошибка была перехвачена:', error.message);
	}
}
...
function makeFourteen() {
	...
	try {
		...
		} else console.log(`Ваша скидка на товар ${discount}%`);
	} catch (error) {
		console.error('Ошибка была перехвачена:', error.message);
		}
}
...
function makeEighteen() {
	...
	//Некорректная операция деления для примера, может вызвать ошибку
		...
		} else resultEighteen.textContent = result;
	} catch (error) {
...
function makeNineteen() {
	...
	try {
		...
		console.error(error);
		resultEighteen.textContent = 'Ошибка'; 
	} finally {
		forWordFinally.textContent = 'finally19';
	}
}
...
function makeTwenty() {
	...
	try {
		...
		forWordFinally.textContent = 'finally20';
	}
}
...
function makeTwentyOne() {
	...
	try {
		...
		forWordFinally.textContent = 'finally21';
	}
}
...
function makeTwentyTwo() {
	...
	try {
		...
		} else throw new Error('Произошла ошибка при получении данных');
	} catch (e) {
...
function makeTwentyFour() {
	...
	return result;
		...
		resultTwentyFour.textContent = e;
	}
}
...
function makeTwentyFive() {
	...
	try {
		...
		resultTwentyFive.textContent = e;
	}
}
...
function makeTwentySix() {
	...
	// const noSuchVariable = 'Проверка';
	try {
		if (typeof noSuchVariable !== 'undefined') {
			setTimeout(() => {
		console.log(noSuchVariable); //Попытка обратиться к несуществующей переменной
		}, 1000);
		} else throw new Error('Переменная noSuchVariable не найдена');
	} catch (e) {
		resultTwentySix.textContent = e;
	}
}
